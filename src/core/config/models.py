...
from src.core.config.enums import ProcessingStage, SubStage, StatusType, validate_stage_substage
...
class LogEntry:
    ...
    def __post_init__(self):
        ...
        if not validate_stage_substage(self.stage, self.substage):
            raise ValueError(
                f"Substage {self.substage} does not belong to stage {self.stage}"
            )
